...
interface GeoLocationAuditSheetProps {
  open: boolean;
...
export function GeoLocationAuditSheet({
  open,
  onOpenChange,
  geoLocationId,
}: GeoLocationAuditSheetProps) {
  const { t } = useTranslation();
  const isMobile = useIsMobile();
  // ESAVI-GEOLOC-003 — reads `appDetails` from the same cached row the list already has.
  const { data } = geoLocationResource.useOne(geoLocationId ?? '');
