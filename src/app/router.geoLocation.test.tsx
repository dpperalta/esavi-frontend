...
// Mirrors the real nesting of app/router.tsx: AppShell → RequireRole(USER) → /geo-locations.
...
describe('Ruta /geo-locations — navegación desde el sidebar', () => {
  it('el enlace del sidebar navega a la pantalla de ubicaciones', async () => {
    const user = userEvent.setup();
    signInAs('ADMIN', 50);

    renderApp('/');

    const link = await screen.findByRole('link', { name: 'Ubicaciones' });
    await user.click(link);

    expect(await screen.findByRole('heading', { name: 'Ubicaciones' })).toBeInTheDocument();
  }, 20000);
});

describe('Ruta /geo-locations — autorización', () => {
  it('con rol ANALYTICS la entrada del menú no aparece', async () => {
    ...
    expect(screen.queryByRole('link', { name: 'Ubicaciones' })).not.toBeInTheDocument();
  });

  it('con rol ANALYTICS, entrar por URL redirige a / sin pantalla en blanco', async () => {
    signInAs('ANALYTICS', 10);

    renderApp('/geo-locations');

    await waitFor(() => expect(screen.getByText(/Hola,/)).toBeInTheDocument());
    expect(screen.queryByRole('heading', { name: 'Ubicaciones' })).not.toBeInTheDocument();
  });
});
