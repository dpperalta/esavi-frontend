...
// Mirrors the real nesting of app/router.tsx: AppShell → RequireRole(USER) → /catalog-types.
...
describe('Ruta /catalog-types — navegación desde el sidebar', () => {
  ...
  it('el enlace del sidebar navega a la pantalla de tipos de catálogo', async () => {
    ...
    const link = await screen.findByRole('link', { name: 'Tipos de catálogo' });
...
describe('Ruta /catalog-types — autorización', () => {
  it('con rol ANALYTICS la entrada del menú no aparece', async () => {
    ...
    expect(screen.queryByRole('link', { name: 'Tipos de catálogo' })).not.toBeInTheDocument();
  });

  it('con rol ANALYTICS, entrar por URL redirige a / sin pantalla en blanco', async () => {
    signInAs('ANALYTICS', 10);

    renderApp('/catalog-types');

    await waitFor(() => expect(screen.getByText(/Hola,/)).toBeInTheDocument());
    expect(screen.queryByRole('heading', { name: 'Tipos de catálogo' })).not.toBeInTheDocument();
  });
});
