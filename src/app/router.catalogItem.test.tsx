...
// Mirrors the real nesting of app/router.tsx: AppShell → RequireRole(USER) → /catalog-items.
...
describe('Ruta /catalog-items — navegación desde el sidebar', () => {
  it('el enlace del sidebar navega a la pantalla de elementos de catálogo', async () => {
    const user = userEvent.setup();
    signInAs('ADMIN', 50);

    renderApp('/');

    const link = await screen.findByRole('link', { name: 'Elementos de catálogo' });
    await user.click(link);

    expect(
      await screen.findByRole('heading', { name: 'Elementos de catálogo' }),
    ).toBeInTheDocument();
  }, 20000);
});

describe('Ruta /catalog-items — autorización', () => {
  it('con rol ANALYTICS la entrada del menú no aparece', async () => {
    ...
    expect(screen.queryByRole('link', { name: 'Elementos de catálogo' })).not.toBeInTheDocument();
  });

  it('con rol ANALYTICS, entrar por URL redirige a / sin pantalla en blanco', async () => {
    signInAs('ANALYTICS', 10);

    renderApp('/catalog-items');

    await waitFor(() => expect(screen.getByText(/Hola,/)).toBeInTheDocument());
    expect(
      screen.queryByRole('heading', { name: 'Elementos de catálogo' }),
