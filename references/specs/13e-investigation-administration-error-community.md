...
- **Geolocalización del navegador, búsqueda por dirección y polígonos.** `<MapPointPicker>` es un marcador arrastrable sobre teselas; `geoPolygon` existe en la tabla y no se dibuja. *Pendiente anotado durante la verificación manual de FE13a:* un cuadro de búsqueda que geocodifique una dirección escrita y centre el mapa ahí (control superior izquierdo, sobre el zoom) sería útil en los dos consumidores de la primitiva — éste y `investigation` (FE13a §3.7) — y no en uno solo; si se retoma, hace falta decidir primero el proveedor de geocodificación (Nominatim es el candidato obvio por compartir proveedor con las teselas) y su política de uso (1 petición/segundo, `User-Agent` identificable, no pensado para volumen alto), como una mejora a `<MapPointPicker>` en `shared/components/`, no una reimplementación por consumidor.
