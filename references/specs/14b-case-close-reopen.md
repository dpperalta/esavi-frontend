...
| 2 | **Un caso en `PENDING_VALIDATION` no tiene salida en la interfaz.** La línea bloquea, pero la pantalla que resuelve la validación (`011`) no existe todavía | Hoy ese estado sólo se alcanza por API, porque tampoco hay pantalla para pedir validación (`010`). El motivo de la línea dice que hay que resolver la validación, sin enlace. Cuando llegue el spec de validación, se añade el enlace. **Cerrado por SPEC FE23 (2026-09-24):** la línea lleva ahora el botón «Resolver validación» (`CaseValidationActions` en modo `resolveOnly`), no un enlace, porque la validación no tiene paso propio. Pedirla (`010`) se hace desde la cabecera del asistente y el detalle del caso |
